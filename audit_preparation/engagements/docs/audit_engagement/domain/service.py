"""Business-domain service seed for Audit Engagement."""

from __future__ import annotations


ARCHETYPE_PROFILE = {'workflow_profile': {'mode': 'case_flow', 'supports_assignment': True, 'supports_escalation': True}, 'reporting_profile': {'supports_snapshots': True, 'supports_outputs': False}, 'integration_profile': {'external_sync_enabled': False}, 'lifecycle_states': ['open', 'in_review', 'closed', 'archived'], 'is_transactional': False}

CONTRACT = {'title_field': 'title', 'status_field': 'workflow_state', 'reference_field': 'reference_no', 'required_fields': ['title', 'workflow_state'], 'field_purposes': {'workflow_state': 'lifecycle_state', 'requester': 'actor_reference', 'owner': 'actor_reference', 'start_date': 'schedule_marker', 'due_date': 'schedule_marker'}, 'search_fields': ['title', 'reference_no', 'description', 'engagement_no', 'audit_type', 'requester'], 'list_columns': ['title', 'reference_no', 'workflow_state', 'modified'], 'initial_state': 'open', 'lifecycle_states': ['open', 'in_review', 'closed', 'archived'], 'terminal_states': ['closed', 'archived'], 'action_targets': {'close': 'closed', 'create': None, 'assign': 'in_review', 'archive': 'archived', 'review': 'in_review'}}

WORKFLOW_HINTS = {'business_objective': 'Act as the anchor case for audit preparation, evidence gathering, findings, and responses.', 'actors': ['audit coordinator', 'owner', 'requester'], 'primary_transitions': ['audit_engagement: open -> in_review -> closed -> archived']}
